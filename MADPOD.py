    ...
    for i in range(1,len(record)):
        if pre != record[i]:
            if record[i] not in temp:
                ...
                res+=1
        pre = record[i]
    print("Analyze"  , file=sys.stderr, flush=True)
...
def addZero(Record):
    for i in range(1,len(Record)):
        if(distance(Record[0][0],Record[0][1] ,Record[i][0],Record[i][1]) <= 600 ):
            Record[0]=Record[i]
    return Record

class StateMachine:
    def __init__(self,x, y, next_checkpoint_x, next_checkpoint_y, next_checkpoint_dist, next_checkpoint_angle,opponent_x, opponent_y):
        self.x=x
        self.y=y
        ...
        self._XDATA=[]
        self._YDATA=[]
        self.brakeobj=Brake(0,1)
        self.maps=[]
        self.myprepos=[0,0]    
        self.oppoprepos=[0,0]
        self.myspeed=[0,0]
        self.oppospeed=[0,0]
    def Execute(self,x, y, next_checkpoint_x, next_checkpoint_y, next_checkpoint_dist, next_checkpoint_angle,opponent_x, opponent_y):
        self.x=x
        self.y=y
        self.next_checkpoint_x=next_checkpoint_x
        self.next_checkpoint_y=next_checkpoint_y
        ...
        self.myspeed[0]=x-self.myprepos[0]
        self.myspeed[1]=y-self.myprepos[1]
        if time >=2:
            self.oppospeed[0]=opponent_x-oppoprepos[0]
            self.oppospeed[1]=opponent_y-oppoprepos[1]
        myprepos=[x,y]
        self.oppoprepos=[opponent_x,opponent_y]
        print(self.maps, file=sys.stderr, flush=True)
        thrust=0
        print(next_checkpoint_angle, file=sys.stderr, flush=True)
        print(next_checkpoint_dist, file=sys.stderr, flush=True)

        if [next_checkpoint_x,next_checkpoint_y] not in self.maps:
            self.maps.append([next_checkpoint_x,next_checkpoint_y])
        self.Record = addZero(self.Record)

        print("Rounds ! %d" %Rounds, file=sys.stderr, flush=True)


        nextnextpos =  nextck([next_checkpoint_x,next_checkpoint_y])
        opponextpos =[int(opponent_x+self.oppospeed[0]) , int(opponent_y+self.oppospeed[1])]
        mynextpos = [int(x+self.myspeed[0]),int(y+self.myspeed[1])]

        if time ==0:
            self.Record.append([x,y])

        self.Record.append([next_checkpoint_x,next_checkpoint_y])
        Rounds= AnalyzeRecord(selfs.Record)
        ...
        myspeedrate = math.sqrt(self.myspeed[0]**2+self.myspeed[1]**2)
        oppospeedrate = math.sqrt(self.oppospeed[0]**2+self.oppospeed[1]**2)
        print("myspeedrate %f " %myspeedrate, file=sys.stderr, flush=True)

        if(next_checkpoint_angle >90 or  next_checkpoint_angle < -90) : 
            thrust=0
            if abs(next_checkpoint_angle) >= 120:
                temp = self.brakeobj.brake([x,y],self.myspeed)
                resx = temp[0]
                resy = temp[1]   
        else:
            thrust=100
        ...
        pridictdistance=distance(x+self.myspeed[0],y+self.myspeed[1] , opponextpos[0],opponextpos[1])
        ...
        if (pridictdistance <=2000 and abs(angle_between_2p(mynextpos,opponextpos) - angle_between_2p(mynextpos , [next_checkpoint_x,next_checkpoint_y])) <=40 and self.Boost < self.Boostmaxs and distance(mynextpos[0],mynextpos[1] , next_checkpoint_x,next_checkpoint_y)>distance(mynextpos[0],mynextpos[1],next_checkpoint_x,next_checkpoint_y)and abs(next_checkpoint_angle) <60  ) :
        ...
        print(self.myspeed, file=sys.stderr, flush=True)
        print(self.oppospeed, file=sys.stderr, flush=True)
        print(self.distance(x,y ,opponent_x,opponent_y), file=sys.stderr, flush=True)
