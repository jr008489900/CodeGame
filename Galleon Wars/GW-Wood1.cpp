...
void Solution::Update(vector<Entity> entities, int my_ship_count)
{
    ...
    for (int i = 0; i < entities.size(); i++)
    {
        ...
        {
            myships.push_back(Ship(entities[i].entity_id, entities[i].entity_type, entities[i].x, entities[i].y, entities[i].arg_1, entities[i].arg_2, entities[i].arg_3, entities[i].arg_4, this));
        }
        else if (entities[i].entity_type == "SHIP" && entities[i].arg_4 == 0)
        {
            enemyships.push_back(Ship(entities[i].entity_id, entities[i].entity_type, entities[i].x, entities[i].y, entities[i].arg_1, entities[i].arg_2, entities[i].arg_3, entities[i].arg_4, this));
        }
    }
}
